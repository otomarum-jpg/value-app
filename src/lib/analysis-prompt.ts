...
1. discoveryCandidates（3つ）:
   - title: 日常で繰り返している行動パターンを表す短い見出し。「責任感がある」「向上心がある」「自分なりの基準で動く」のような性格名詞・抽象ラベルは禁止。行動として頭の中で映像が浮かぶ表現にする。（NG:「向上心がある」→OK:「できると思ったことができないと、他人より自分に対して悔しくなる」／NG:「自分なりの基準で動く」→OK:「周りが良いと言っても、自分に意味を感じなければ動かない」）
